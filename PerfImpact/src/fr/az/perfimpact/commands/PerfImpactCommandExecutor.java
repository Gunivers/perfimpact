	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equals("perfimpact")) {
			...
			if (args[0].equals("capture")) {
				if (args.length == 1) {
					sender.sendMessage(
							ChatColor.GOLD + "Capture enabled: " + ChatColor.DARK_GREEN + main.isCapturing());
					return true;
				}
				...
				if (args[1].equals("start")) {
					...
					cmd.execute(sender, label, new String[] { "help", "capture" });
			...
			if (args[0].equals("stop")) {
				...
				double exeTime = main.timer.getOrDefault(path.toString(), new TimerTask()).getTime();

				ArrayList<Double> timeImpact = main.timeImpact.getOrDefault(path.toString(), new ArrayList<Double>());
				timeImpact.add(exeTime);
				main.timeImpact.put(path.toString(), timeImpact);

				main.timer.remove(path.toString());

				sender.sendMessage(ChatColor.GOLD + "The function " + path + " was executed in " + ChatColor.DARK_GREEN
						+ exeTime + "s");
				return true;
			}
			...
			if (args[0].equals("resume")) {
				...
				if (args.length == 1) {
					...
					for (ArrayList<Double> temp : main.timeImpact.values()) {
						for (double impact : temp) {
							totalExeTime += impact;
						}
					}
									...
									if (child.getPath().equals(tag)) {
										...
										if (child.getPath().endsWith(".mcfunction")) {
											...
											for (int i = 0; i < child.getCallCount(); i++) {
												child.setExecutionTime(
														child.getExecutionTime() + main.timeImpact.get(pathway).get(i));
											}
					...
					double mean = 0;
					for (double exeTime : main.timeImpact.get(path.toString())) {
						mean += exeTime;
					}
					mean /= main.timeImpact.get(path.toString()).size();

					long callCount = main.timeCalled.get(path.toString());
					ArrayList<Double> exeTime = main.timeImpact.get(path.toString());

					sender.sendMessage(ChatColor.DARK_PURPLE + "Function " + path + "\n" + ChatColor.GOLD
							+ "Mean of execution time: " + ChatColor.DARK_RED + mean + ChatColor.GOLD
							+ "; Total Calls: " + ChatColor.DARK_RED + callCount);
					sender.sendMessage(ChatColor.GOLD + "Execution Times: ");

					for (double i : exeTime) {
			...
			if (args[0] == "help") {
				sender.sendMessage(ChatColor.DARK_GREEN + "The help of '" + ChatColor.GOLD + path + ChatColor.DARK_GREEN
						+ "' is coming soon !");
				return true;
			}
		}
